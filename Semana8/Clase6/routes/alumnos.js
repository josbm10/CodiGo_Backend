...
function alumnosApi(app){
    ...
    router.post("/",async function(req,res,next){
        ...
        try{
            const createAlumnoId = await alumnosService.create({alumno});
            res.status(201).json({
                status:true,
    ...
    router.put("/:alumnoId",async function(req,res,next){
        ...
        const{body:alumno}=req;
        try{
            const updateAlumnoId = await alumnosService.update({alumno,alumnoId});
            res.status(200).json({
                status:true,
    ...
    router.delete("/:alumnoId",async function(req,res,next){
        ...
        try{
            const deleteAlumnoId = await alumnosService.delete({alumnoId});
            res.status(200).json({
                status:true,
